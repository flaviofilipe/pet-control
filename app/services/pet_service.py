...
class PetService:
    ...
    def get_user_pets(self, user_id: str) -> List[Dict[str, Any]]:
        """Busca pets do usuário"""
        return self.pet_repo.get_pets_by_user(user_id)
    
    def get_pet_details(self, pet_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Busca detalhes de um pet específico"""
        return self.pet_repo.get_pet_by_id(pet_id, user_id)
    
    def search_pet_by_nickname(self, nickname: str, requesting_user_id: str) -> Tuple[bool, Optional[Dict[str, Any]], str]:
        """
        Busca pet por nickname e verifica acesso do usuário
        Retorna: (sucesso, pet_data, mensagem)
        """
        pet = self.pet_repo.get_pet_by_nickname(nickname)
    ...
    def create_pet(self, pet_data: Dict[str, Any], user_id: str) -> Tuple[bool, str, Optional[str]]:
        """
        Cria um novo pet
        ...
        nickname = self._generate_unique_nickname(base_name)
        ...
        try:
            pet_id = self.pet_repo.create_pet(pet_document)
            return True, "Pet criado com sucesso!", pet_id
        except Exception as e:
            return False, f"Erro ao criar pet: {str(e)}", None
    
    def update_pet(self, pet_id: str, pet_data: Dict[str, Any], user_id: str) -> Tuple[bool, str]:
        """
    ...
    def delete_pet(self, pet_id: str, user_id: str) -> Tuple[bool, str]:
        ...
        """
        try:
            success = self.pet_repo.soft_delete_pet(pet_id, user_id)
            if success:
    ...
    def grant_veterinarian_access(self, pet_id: str, vet_id: str, owner_id: str) -> Tuple[bool, str]:
        """
        Concede acesso de veterinário ao pet
        Retorna: (sucesso, mensagem)
        """
        # Verifica se o pet existe e se o usuário é o dono
        pet = self.pet_repo.get_pet_by_id(pet_id, owner_id)
        if not pet:
        ...
        try:
            was_granted = self.pet_repo.grant_vet_access(pet_id, vet_id)
            if was_granted:
    ...
    def revoke_veterinarian_access(self, pet_id: str, vet_id: str, owner_id: str) -> Tuple[bool, str]:
        """
        ...
        try:
            was_revoked = self.pet_repo.revoke_vet_access(pet_id, vet_id)
            if was_revoked:
    ...
    def get_pet_veterinarians(self, pet_id: str, owner_id: str) -> Tuple[bool, List[Dict[str, Any]], str]:
        """
        ...
        try:
            veterinarians = self.user_repo.get_veterinarians_by_ids(veterinarian_ids)
    ...
    def add_treatment(self, pet_id: str, user_id: str, treatment_data: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        if treatment_data.get("applier_type") == "Veterinarian":
            self.pet_repo.grant_vet_access(pet_id, user_id)
        
        try:
            success = self.pet_repo.add_treatment(pet_id, user_id, treatment_data)
            if success:
    ...
    def update_treatment(self, pet_id: str, treatment_id: str, user_id: str, treatment_data: Dict[str, Any]) -> Tuple[bool, str]:
        """
        Atualiza tratamento do pet
        Retorna: (sucesso, mensagem)
        """
        try:
            success = self.pet_repo.update_treatment(pet_id, user_id, treatment_id, treatment_data)
            if success:
    ...
    def delete_treatment(self, pet_id: str, treatment_id: str, user_id: str) -> Tuple[bool, str]:
        """
        Remove tratamento do pet
        Retorna: (sucesso, mensagem)
        """
        try:
            success = self.pet_repo.delete_treatment(pet_id, user_id, treatment_id)
            if success:
    ...
    def _generate_unique_nickname(self, base_name: str) -> str:
        ...
        while attempts < max_attempts:
            ...
            if not self.pet_repo.check_nickname_exists(candidate_nickname):
